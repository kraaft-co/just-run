...
/**
 * Removing the lock in place empties it before deleting it, and another
 * process can rename its own lock over the empty folder in between. Moving it
 * aside first is atomic: the lock is either still whole or gone.
 */
function removeLock(lockDir) {
  const discardedDir = `${lockDir}.${process.pid}.discarded`;
  removeDir(discardedDir);
  try {
    nodeFs.renameSync(lockDir, discardedDir);
  } catch (e) {
    if (e.code === "ENOENT") {
      return;
    }
...
function tryAcquire(lockDir) {
  ...
  try {
    ...
    removeDir(preparedDir);
    // The lock may be released right after the failed rename: that is still
    // a lost race, the caller tries again.
    if (e.code === "EEXIST" || e.code === "ENOTEMPTY") {
      return false;
    }
    // Windows reports an existing target as EPERM, as it does a real
    // permission error.
    if (e.code === "EPERM" && nodeFs.existsSync(lockDir)) {
      return false;
    }
    throw e;
  }
...
/**
 * Returns a function that tries once to take the lock and, when it is held,
 * removes it if stale and warns once the wait gets long. The sync and async
 * variants call it in a loop, and only differ in how they sleep between tries.
 */
function createLockTaker(lockDir, timeout = DEFAULT_TIMEOUT_MS) {
 ...
 * @param {() => T} fn
 * @param {number} [timeout] - How long to wait before warning, and before
 *   taking over a lock whose holder cannot be identified.
 * @return {T}
 * @template T
 */
function withBuildLockSync(cwd, fn, timeout) {
  const lockDir = nodePath.join(cwd, LOCK_DIR_NAME);
  const tryTake = createLockTaker(lockDir, timeout);
  while (!tryTake()) {
 ...
 * @template T
 */
async function withBuildLock(cwd, fn, timeout) {
  const lockDir = nodePath.join(cwd, LOCK_DIR_NAME);
  const tryTake = createLockTaker(lockDir, timeout);
  while (!tryTake()) {
